...
class UAMFormTestCase(TestCase):
    ...
    def test_no_valid_option(self):
        """ Should fail if the selected option is 'Selecciona una opción' """
    ...
    def test_external_user_without_email(self):
        """ Should pass if the user selects 'No soy de la UAM' """
    ...
    def test_student_with_valid_email(self):
        """ Should pass if the student uses an @estudiante.uam.es email """

        form = UAMForm(
            data={"user_choice": "2", "email_uam": "user@estudiante.uam.es"})
        self.assertTrue(form.is_valid())

    def test_student_with_invalid_email(self):
        """ Should fail if a student uses an incorrect email """

        form = UAMForm(data={"user_choice": "2", "email_uam": "user@uam.es"})
        self.assertFalse(form.is_valid())

        form = UAMForm(
            data={"user_choice": "2", "email_uam": "user@gmail.com"})
        self.assertFalse(form.is_valid())

    # Option 3
    def test_professor_researcher_with_valid_email(self):
        """ Should pass if the professor or researcher uses an @uam.es email """

        form = UAMForm(data={"user_choice": "3", "email_uam": "user@uam.es"})
        self.assertTrue(form.is_valid())

    def test_professor_researcher_with_invalid_email(self):
        """ Should fail if the professor or researcher uses an incorrect email """

        form = UAMForm(
            data={"user_choice": "3", "email_uam": "user@estudiante.uam.es"})
        self.assertFalse(form.is_valid())

        form = UAMForm(
            data={"user_choice": "3", "email_uam": "user@gmail.com"})
        self.assertFalse(form.is_valid())

    # Option 4
    def test_administrative_staff_with_valid_email(self):
        """ Should pass if the administrative staff uses an @uam.es email """

        form = UAMForm(data={"user_choice": "4", "email_uam": "user@uam.es"})
        self.assertTrue(form.is_valid())

    def test_administrative_staff_with_invalid_email(self):
        """ Should fail if the administrative staff uses an incorrect email """

        form = UAMForm(
            data={"user_choice": "4", "email_uam": "user@estudiante.uam.es"})
        self.assertFalse(form.is_valid())

        form = UAMForm(
            data={"user_choice": "4", "email_uam": "user@gmail.com"})
        self.assertFalse(form.is_valid())
    ...
    def test_old_student_with_valid_email(self):
        ...
        form = UAMForm(
            data={"user_choice": "5", "email_uam": "user@alumni.uam.es"})
        self.assertTrue(form.is_valid())
    ...
    def old_student_with_invalid_email(self):
        ...
        form = UAMForm(
            data={"user_choice": "5", "email_uam": "user@estudiante.uam.es"})
        self.assertFalse(form.is_valid())
        ...
        form = UAMForm(
            data={"user_choice": "5", "email_uam": "user@gmail.com"})
        self.assertFalse(form.is_valid())
