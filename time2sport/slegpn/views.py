...
def complete_enrollment(request, bonus_id):
    ...
    if bonus.bonus_type == 'single':
        ...
        product_bonus = ProductBonus.objects.create(
            user=user, bonus=bonus, one_use_available=True)
        ...
        product_bonus = ProductBonus.objects.create(
            user=user, bonus=bonus, date_begin=inicio, date_end=fin)
        Notification.objects.create(user=user, title=title, content=content)
...
def cancel_waiting_list(request, waiting_list_id):
    ...
    if request.method == "POST":
        ...
        waiting_entry = get_object_or_404(
            WaitingList, id=waiting_list_id, user=request.user)
...
def join_waiting_list(request, session_id):
    ...
    if request.method == "POST":
        ...
        if session.free_places > 0 and not session.waiting_list.exists():
            messages.error(
                request, "La sesión no está llena, puedes reservar directamente.")
        ...
        if request.user.reservations.filter(session=session).exists():
            messages.info(
                request, "Ya tienes una reserva para esta sesión, no te puedes apuntar a la lista de espera.")
            return redirect('activity_detail', session.activity.id)
        ...
        if request.user.waiting_lists.filter(session=session).exists():
            messages.info(
                request, "Ya estás en la lista de espera para esta sesión.")
            return redirect('activity_detail', session.activity.id)

        # Check if the user does not have another session at the same time
        users_sessions_day = request.user.reservations.filter(
        ...
        if start_time <= timezone.now() < end_time:
            messages.error(
                request, "La sesión ya ha comenzado, no te puedes apuntar a la lista de espera.")
            return redirect('activity_detail', session.activity.id)
        elif timezone.now() >= end_time:
            messages.error(
                request, "La sesión ya ha finalizado, no te puedes apuntar a la lista de espera.")
            return redirect('activity_detail', session.activity.id)

        # Add to waiting list and notify
        WaitingList.objects.create(user=request.user, session=session)
        messages.success(
            request, "Te has apuntado correctamente a la lista de espera.")
        waiting_list = WaitingList.objects.filter(
            session=session).order_by('join_date')
        position = list(waiting_list).index(
            request.user.waiting_lists.get(session=session)) + 1
