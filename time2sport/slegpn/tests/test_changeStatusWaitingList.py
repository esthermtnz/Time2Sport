...
class WaitingListNotificationTestCase(TestCase):
    ...
    def test_remove_from_waiting_list_notification(self):
        ...
        self.client.post(reverse('cancel_waiting_list', args=[
                         waiting_entry.id]), follow=True)

        # Check if the notification is received
        self.notification = self.client.get(reverse('notifications'))
        self.assertEqual(self.notification.status_code, 200)
        self.assertContains(self.notification,
                            "Has sido eliminado de la lista de espera")
...
class ChangeWaitingListStatusTestCase(TestCase):
    ...
    def test_notify_next_user_when_place_freed(self):
        ...
        self.client.login(username="testuser", password="testpassword")
        # Reserve session
        self.client.post(reverse('reserve_activity_session', args=[
        ...
        self.client.post(reverse('cancel_reservation', args=[
                         reservation.id]), follow=True)
        ...
        self.assertEqual(self.notification.status_code, 200)
        self.assertContains(self.notification,
                            "¡Apúntate a la sesión, se ha liberado una plaza!")
        ...
        self.assertNotContains(
            self.notification, "¡Apúntate a la sesión, se ha liberado una plaza!")
    ...
    def test_notify_second_place_if_first_cannot(self):
        ...
        self.notification = self.client.get(reverse('notifications'))
        self.assertContains(self.notification,
                            f"Tiempo para reservar a expirado")
