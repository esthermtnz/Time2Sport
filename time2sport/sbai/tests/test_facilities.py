...
class AllFacilitiesViewTestCase(TestCase):
    ...
    def test_redirect_for_unauthenticated_users(self):
        ...
        self.assertRedirects(response, f"/accounts/login/?next={reverse('all_facilities')}")
    ...
    def test_facility_without_schedule(self):
        ...
        self.assertIn(self.sport_facility_without_schedule, response.context['facilities'])

        #-- Template verification --
        self.assertContains(response, 'Instalaciones Deportivas')
        self.assertContains(response, self.sport_facility_without_schedule.name)
        self.assertContains(response, self.sport_facility_without_schedule.get_facility_type_display())
        self.assertContains(response, self.sport_facility_without_schedule.hour_price)
        self.assertContains(response, self.sport_facility_without_schedule.number_of_facilities)
        
        #Check that the 'Reservar' button is displayed and redirects to facility details
        reservar_btn = reverse('facility_detail', args=[self.sport_facility_without_schedule.id])
        self.assertContains(response, f'href="{reservar_btn}"')
        ...
        if self.sport_facility_without_schedule.photos.exists():
            self.assertContains(response, self.sport_facility_without_schedule.photos.first().image.url)
        else:
            self.assertContains(response, '/static/default-facility.png')
...
class FacilityDetailViewTestCase(TestCase):
    ...
    def test_facility_without_schedule_detail_view(self):
        ...
        #-- View verification --
        response = self.client.get(reverse('facility_detail', args=[self.sport_facility_without_schedule.id]))
        ...
        self.assertContains(response, self.sport_facility_without_schedule.hour_price)
        self.assertContains(response, self.sport_facility_without_schedule.number_of_facilities)
        self.assertContains(response, self.sport_facility_without_schedule.description)
