...
class AllActivitiesViewTestCase(TestCase):
    ...
    def test_all_activities_view(self):
        ...
        self.assertContains(
            response, self.activity.get_activity_type_display())
        ...
        if self.activity.photos.exists():
            self.assertContains(
                response, self.activity.photos.first().image.url)
        else:
    ...
    def test_activity_without_schedule(self):
        ...
        # Check that activities is sent to the template
        self.assertIn(self.activity_without_schedule,
        ...
        self.assertContains(
            response, self.activity_without_schedule.get_activity_type_display())

        # Check that the 'Reservar' button is displayed and redirects to activity_without_schedule details
        reservar_btn = reverse('activity_detail', args=[
        ...
        if self.activity_without_schedule.photos.exists():
            self.assertContains(
                response, self.activity_without_schedule.photos.first().image.url)
        else:
            self.assertContains(response, '/static/default-activity.jpg')
...
class ActivityDetailViewTestCase(TestCase):
    ...
    def test_activity_detail_view(self):
        ...
        for bonus in self.activity.bonuses.all():
            self.assertContains(
                response, f'{bonus.get_bonus_type_display()} - {bonus.price}€')

        # Check that the 'Inscribirse' button is displayed
        self.assertContains(
            response, '<button class="btn btn-success btn-lg">INSCRIBIRSE</button>', html=True)
    ...
    def test_activity_without_schedule_detail(self):
        ...
        self.assertEqual(self.activity_without_schedule,
                         response.context['activity'])
        ...
        self.assertContains(
            response, self.activity_without_schedule.get_activity_type_display())
        self.assertContains(
            response, self.activity_without_schedule.description)
    ...
    def test_activity_with_multiple_bonuses(self):
        ...
        self.assertContains(
            response, f'{self.bonus1.get_bonus_type_display()} - {self.bonus1.price:.2f}€')
        self.assertContains(
            response, f'{self.bonus2.get_bonus_type_display()} - {self.bonus2.price:.2f}€')
